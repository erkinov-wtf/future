...
use App\Http\Livewire\Admin\Miscellaneous\Comingsoon;
...
use App\Http\Controllers\Appointment\IndexController;
use App\Http\Livewire\User\Appointment\Index as UserIndex;
use App\Http\Livewire\Admin\Appointment\Index as AdminIndex;
use App\Http\Livewire\Doctor\Appointment\Index as DoctorIndex;
...
Route::prefix(config('admintw.prefix'))->middleware(['auth', 'verified', 'activeUser', 'IpCheckMiddleware'])->group(function () {
    ...
    Route::prefix('appointments')->group(function () {
        Route::get('/', IndexController::class)->name('appointments.role-check');
        Route::group(['middleware' => ['role:admin']], function () {
            Route::get('/admin', AdminIndex::class)->name('appointments.admin');
        });
        Route::group(['middleware' => ['role:doctor']], function () {
            Route::get('/doctor', DoctorIndex::class)->name('appointments.doctor');
        });
        Route::group(['middleware' => ['role:user']], function () {
            Route::get('/user', UserIndex::class)->name('appointments.user');
        });
    });

    Route::get('soon', Comingsoon::class)->name('soon');
});
